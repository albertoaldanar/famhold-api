...
export const createFamily = async (req, res) => {
  ...
  if (!name || typeof name !== 'string') {
    return res.status(400).json({ message: 'Family name is required and must be a string.' });
  }

  if (!accountManagerId || typeof accountManagerId !== 'number') {
    return res.status(400).json({ message: 'Account Manager ID is required and must be a number.' });
  }

  const familyId = `fam_${name.toLowerCase().replace(/\s+/g, '_')}_vfo`;

  try {
    const encryptedPayload = encryptObject({ name, familyId });
    ...
    return res.status(500).json({ message: 'Error creating family and VFO.', error });
  }
};
